...
const PLANNER_INSTRUCTION = 'You are an agent-routing planner. Analyze every distinct task in the user request and assign each task to the best specialist, in the same requested order. Available specialists: comedian, doctor, detective. If a prompt asks for a joke AND health information, include both comedian and doctor; never merge distinct tasks into one task. Return only valid JSON in this exact shape: {"agents":["comedian","doctor"],"tasks":{"comedian":"the exact joke task","doctor":"the exact health task"}}. Include only specialists with a meaningful task. Each task must contain only that specialist portion, with references such as that resolved to their subject. Never answer the user or explain your choices.';
